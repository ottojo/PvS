...
public class CtrlGroup {
    ...
    public static void setNewWorldCup(SoccerWC worldCup, Vector<DefaultListModel> models, String name)
        ...
        for (Group group : groups) {
            ...
            games.clear();
            for (int j = 0; j < 6; j++) {
                games.add(new Game(Integer.valueOf(br.readLine()).intValue(), br.readLine(), br.readLine(),
                        br.readLine(), teams.get(Integer.valueOf(br.readLine()).intValue() - 1),
                        teams.get(Integer.valueOf(br.readLine()).intValue() - 1), 0, 0, false));
            }
            i++;
        }
    ...
    public static boolean loadServerFile(SoccerWC worldCup, String filename) {
        try {
            ReadWC.readFromServer(filename, worldCup);
        } catch (Exception e) {
            e.printStackTrace(System.err);
            return false;
        }
